 #!/usr/bin/env python3
"""
app.py - Flask application for Dissolution Tester
...
ESP_PORT = os.environ.get("ESP_PORT", os.environ.get("ESP_CMD_PORT", "/dev/serial0"))
ESP_BAUD = int(os.environ.get("ESP_BAUD", os.environ.get("ESP_CMD_BAUD", "9600")))
ESP_CMD_PORT = os.environ.get("ESP_CMD_PORT", ESP_PORT)
ESP_CMD_BAUD = int(os.environ.get("ESP_CMD_BAUD", str(ESP_BAUD)))
ESP_TEMP_PORT = os.environ.get("ESP_TEMP_PORT", "/dev/ttyAMA3")
ESP_TEMP_BAUD = int(os.environ.get("ESP_TEMP_BAUD", "9600"))
...
EXPORT_SUBFOLDER = "Dissolution-Tester-Reports-Exported"
...
config = {
    ...
    "SIMULATE_HARDWARE": str(os.environ.get("SIMULATE_HARDWARE", "0")).strip().lower() in ("1", "true", "yes", "on"),
}
...
import disso_cmd_hardware
import disso_temp_hardware
import disso_test_service

disso_cmd_hardware.init(app, config)
disso_temp_hardware.init(app, config)
disso_test_service.init(app.logger, config)
...
def _apply_power_loss_abort_to_report(report: dict) -> dict:
    ...
    # Keep approval pending so aborted runs still require reviewer/Admin sign-off.
...
def _audit_power_loss_aborted_report(report: dict) -> None:
    """Audit row for a report saved as power-loss aborted and pending approval."""
    rid = report.get("id")
    if rid is None:
        return
    ctx = _format_report_audit_details(int(rid), report)
    pl_detail = "{} | unclean shutdown | remarks: {} | awaiting approval".format(
        ctx,
...
def _startup_session_power_audit():
    ...
    try:
        ...
        unclean_session = bool(pending)
        # Power cut with active test: recover even if session-pending file is missing.
        needs_recovery = active_checkpoint or unclean_session

        if unclean_session and not pending.get("powerAuditLogged"):
            ...
            try:
                ...
                if recovery.get("aborted") and recovery.get("checkpoint"):
                    ...
                    try:
                        ...
                        pass
                    report_data = dict(cp)
                    report_data["abortReason"] = "power_failure_timeout"
                    ...
                    enriched = _stamp_report_operator(enriched)
                    if report_data.get("operators"):
                        enriched["operatorTrail"] = report_data.get("operators")
                        enriched.setdefault("testData", {})["operatorTrail"] = report_data.get("operators")
                    started = report_data.get("startedBy") or {}
                    if started.get("name") or started.get("username"):
                        enriched["operatorName"] = started.get("name") or started.get("username")
                        enriched["operatedByUsername"] = started.get("username")
                    enriched = _apply_power_loss_abort_to_report(enriched)
                    rid = data_service.save_report(enriched)
                    enriched["id"] = rid
                    data_service.save_report(enriched)
                    try:
                        print_service.save_report_text_files(enriched, int(rid), REPORTS_DIR)
                    except Exception:
                        pass
                    _audit_power_loss_aborted_report(enriched)
                    data_service.clear_test_run_data()
                elif not recovery.get("recovered") and not recovery.get("pending") and unclean_session:
                    un = (pending or {}).get("username") or ""
                    _abort_pending_reports_after_power_loss(un)
            except Exception:
                app.logger.exception("Dissolution power recovery failed")
                try:
                    un = (pending or {}).get("username") or ""
                    _abort_pending_reports_after_power_loss(un)
...
PERMISSION_CARD_LABELS = {
    ...
    "perm_calibration_access": "Calibration access",
    "perm_calibration_report_approve": "Calibration report approval",
    "perm_datetime": "Edit date and time",
    "perm_system_settings": "Test Settings",
    "perm_wakeup_schedule": "Wakeup Schedule",
    "perm_cleaning_cycle": "Cleaning Cycle",
...
def serve_static(path):
    # Never treat API paths as static files (avoids misleading 405s).
    if str(path or "").replace("\\", "/").startswith("api/"):
...
def approve_report(report_id):
    try:
        ...
        report["drumPassFail"] = {"drum1": pf, "drum2": pf}
...
def abort_report(report_id):
    try:
        ...
        report["testData"] = td
        report["status"] = "aborted"
        # Keep aborted reports in pending state to enforce approval workflow.
        report["reportApprovalStatus"] = "pending"
        for k in ("approvalPassFail", "approvalRemarks", "approvedBy", "approvedAt", "approvedByUsername"):
            report.pop(k, None)
        if not report.get("completedAt"):
        ...
        data_service.save_report(report)
        ctx = _format_report_audit_details(report_id, report)
        abort_detail = "{} | awaiting approval".format(ctx)
...
@app.route("/api/data/system-settings", methods=["GET"])
def get_system_settings():
    try:
        gate = _require_session_internal(
            "system-settings",
            "Forbidden. You do not have permission to view system settings.",
        )
        if gate:
            return gate
        settings = data_service.get_system_settings()
        return jsonify({"settings": settings}), 200
    except Exception as e:
        app.logger.exception("Error getting system settings")
        return jsonify({"error": str(e)}), 500


@app.route("/api/data/system-settings", methods=["POST"])
def save_system_settings():
    try:
        gate = _require_session_internal(
            "system-settings",
            "Forbidden. You do not have permission to save system settings.",
        )
        if gate:
            return gate
        settings = request.get_json(force=True, silent=True) or {}
...
def system_network_addresses():
    try:
        gate = _require_session_internal(
            "settings",
            "Forbidden. You do not have permission to view network addresses.",
        )
        if gate:
            return gate
        ...
        app.logger.exception("Error listing network addresses")
        return jsonify({"ok": False, "error": str(e)}), 500
...
def system_info():
    try:
        gate = _require_session_internal(
            "settings",
            "Forbidden. You do not have permission to view system info.",
        )
        if gate:
            return gate
        ...
        live = {}
        try:
            live = disso_temp_hardware.get_live() or {}
        except Exception as live_err:
            app.logger.warning("system info temps: %s", live_err)

        test_st = {}
        try:
            test_st = disso_test_service.get_state() or {}
        except Exception as st_err:
            app.logger.warning("system info test state: %s", st_err)

        statues = (live.get("statues") or live.get("status") or {}) if isinstance(live, dict) else {}
        if not isinstance(statues, dict):
            statues = {}

        hw = {}
        try:
            hw = hardware_service.cmd_system_info() or {}
        except Exception as hw_err:
            app.logger.warning("system info legacy status: %s", hw_err)
            hw = {"ok": False, "error": str(hw_err)}

        bath = live.get("bath")
        if bath is None:
            bath = hw.get("bath_c")

        machine_state = statues.get("state") or test_st.get("runStatus") or "IDLE"
        if not test_st.get("active") and not statues.get("state"):
            machine_state = "IDLE"

        out = {
            "ok": True,
            "bath": hardware_service._format_bath_c(bath) if bath is not None else "—",
            "bath_c": bath,
            "temps": {
                "bath": live.get("bath"),
                "external": live.get("external"),
                "ext": live.get("external"),
                "vessels": list(live.get("vessels") or [None] * 6),
            },
            "vessels": list(live.get("vessels") or [None] * 6),
            "machineState": machine_state,
            "runStatus": test_st.get("runStatus"),
            "stepCurrent": statues.get("stepCurrent") or (
                (int(test_st.get("stepIndex") or 0) + 1) if test_st.get("active") else None
            ),
            "stepTotal": statues.get("stepTotal") or test_st.get("stepCount"),
            "remainingTime": statues.get("remainingTime"),
            "float_switch": hw.get("float_switch") or "—",
            "heater_sensor": hw.get("heater_sensor") or "—",
            "pump": hw.get("pump") or "—",
            "sample_collector": hw.get("sample_collector") or "—",
            "simulate": bool(live.get("simulate")),
        }
        if hw.get("error") and not hw.get("ok") and not hw.get("placeholder"):
            out["legacyStatusError"] = hw.get("error")
        return jsonify(out), 200
    except Exception as e:
        app.logger.exception("Error getting system info")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/data/wakeup-schedule", methods=["GET", "POST"])
def wakeup_schedule():
    try:
        if request.method == "GET":
            gate = _require_session_internal(
                "wakeup-schedule",
                "Forbidden. You do not have permission to view wakeup schedule.",
            )
            if gate:
                return gate
            settings = data_service.get_wakeup_schedule()
            return jsonify({"settings": settings}), 200

        gate = _require_session_internal(
            "wakeup-schedule",
            "Forbidden. You do not have permission to save wakeup schedule.",
        )
        if gate:
            return gate
        settings = request.get_json(force=True, silent=True) or {}
        saved = data_service.save_wakeup_schedule(settings)
        _audit(None, None, "Wakeup schedule changed", "")
        return jsonify({"success": True, "settings": saved}), 200
    except Exception as e:
        app.logger.exception("Error handling wakeup schedule")
        return jsonify({"error": str(e)}), 500
...
# =================== FACTORY SUPPORT / IP CONFIG ==========================
# Kiosk user needs passwordless sudo for:
...
def factory_support_ping():
    """Sanity check that factory support / IP Config API is loaded."""
...
def _build_audit_trail_html(entries, filters, factory):
    ...
    return (
        ...
        '    <h2>' + (company or "Dissolution Tester") + '</h2>'
        '  </div>'
        '  <div class="meta">'
...
def print_thermal():
    """Thermal printer removed — AMA3 is reserved for Dissolution temp ESP."""
    return jsonify({
        "success": False,
...
def print_status():
    try:
        ...
        if str(printer_type).strip().lower() == "thermal":
            return jsonify({
                "ok": False,
                "available": False,
                "error": "Thermal printer is not available on Dissolution Tester",
...
def friability_disabled():
    return jsonify({"ok": False, "error": "Friability hardware is not part of Dissolution Tester"}), 410


@app.route("/api/hardware/temperature/live", methods=["GET"])
def temperature_live():
    """Prefer Dissolution UART-2 live temps; fall back to legacy hardware_service."""
    gate = _require_any_session_internal(
    ...
    if gate:
        return gate
    try:
        live = disso_temp_hardware.get_live()
        return jsonify({"ok": True, "data": live, "bath": live.get("bath"), "external": live.get("external"), "vessels": live.get("vessels")}), 200
    except Exception:
        state = hardware_service.cmd_get_temperatures()
        return jsonify(state), (200 if state.get("ok") else 400)


@app.route("/api/hardware/validation/load/start", methods=["POST"])
def validation_load_start():
    """Deprecated — friability validation not part of Dissolution."""
    return jsonify({"ok": False, "error": "Not part of Dissolution Tester"}), 410
...
def _disso_save_report_from_run(run: dict, aborted: bool = False):
    """Persist completion/abort report from disso_test_service."""
    recipe = run.get("recipe") or {}
    operators = run.get("operators") or []
    started = run.get("startedBy") or {}
    td = dict(run.get("testData") or {})
    td.update(
        {
            "productName": recipe.get("productName") or recipe.get("name"),
            "powerFailure": run.get("powerFailureMinutes") or recipe.get("powerFailure"),
            "usp": recipe.get("usp") or run.get("usp"),
            "mode": recipe.get("mode") or run.get("mode"),
            "temperature": recipe.get("temperature"),
            "mediaVolume": recipe.get("mediaVolume"),
            "mediaPh": recipe.get("mediaPh"),
            "media": recipe.get("media"),
            "steps": recipe.get("steps"),
            "stepIndex": run.get("stepIndex"),
            ...
            "testStartTime": run.get("startedAtRtc"),
    ...
    payload = {
        ...
        "operatorTrail": operators,
        "startedBy": started,
        "operatorName": started.get("name") or started.get("username") or td.get("operatorName"),
    ...
    enriched = report_service.generate_report(payload, recipe=recipe)
    enriched = _stamp_report_operator(enriched)
    enriched["operatorTrail"] = operators
    enriched.setdefault("testData", {})["operatorTrail"] = operators
    if started:
        enriched["operatorName"] = started.get("name") or started.get("username") or enriched.get("operatorName")
        enriched["operatedByUsername"] = started.get("username") or enriched.get("operatedByUsername")
    if aborted:
        enriched = _apply_power_loss_abort_to_report(enriched) if run.get("abortReason") == "power_failure_timeout" else enriched
        if run.get("abortReason") != "power_failure_timeout":
            enriched["reportApprovalStatus"] = "pending"
            enriched.setdefault("testData", {})["status"] = "aborted"
    report_id = data_service.save_report(enriched)
    enriched["id"] = report_id
    data_service.save_report(enriched)
    try:
        print_service.save_report_text_files(enriched, int(report_id), REPORTS_DIR)
    except Exception:
        pass
    return report_id
...
def disso_test_state():
    try:
        public = str(request.args.get("public", "")).lower() in ("1", "true", "yes")
        # Allow unauthenticated public peek for login modal after auth bootstrap
        if not public:
            gate = _require_auth()
            if gate:
                # still allow active flag for login flow
                st = disso_test_service.get_state(public=True)
                return jsonify({"ok": True, "state": st}), 200
        st = disso_test_service.get_state(public=public)
        return jsonify({"ok": True, "state": st}), 200
    except Exception as e:
        app.logger.exception("disso test state")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/disso/test/start", methods=["POST"])
def disso_test_start():
    try:
        gate = _require_auth()
        if gate:
            return gate
        ...
        meta = {
            "arNumber": body.get("arNumber"),
            "batchNumber": body.get("batchNumber"),
            "powerFailure": body.get("powerFailure"),
        }
        result = disso_test_service.start(recipe, user, meta=meta)
        code = 200 if result.get("ok") else 400
        return jsonify(result), code
    except Exception as e:
        app.logger.exception("disso test start")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/disso/test/pause", methods=["POST"])
def disso_test_pause():
    try:
        gate = _require_auth()
        if gate:
            return gate
        user = data_service.get_current_user() or {}
        result = disso_test_service.pause(user)
        return jsonify(result), 200 if result.get("ok") else 400
    except Exception as e:
        app.logger.exception("disso test pause")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/disso/test/resume", methods=["POST"])
def disso_test_resume():
    try:
        gate = _require_auth()
        if gate:
            return gate
        user = data_service.get_current_user() or {}
        result = disso_test_service.resume_esp(user)
        return jsonify(result), 200 if result.get("ok") else 400
    except Exception as e:
        app.logger.exception("disso test resume")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/disso/test/continue", methods=["POST"])
def disso_test_continue():
    try:
        gate = _require_auth()
        if gate:
            return gate
        gate = _require_any_session_internal(
            ["quick-test", "recipe-test"],
            "Forbidden. You do not have permission to continue tests.",
        )
        if gate:
            return gate
        user = data_service.get_current_user() or {}
        result = disso_test_service.continue_run(user)
        return jsonify(result), 200 if result.get("ok") else 400
    except Exception as e:
        app.logger.exception("disso test continue")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/disso/test/abort", methods=["POST"])
@app.route("/api/disso/test/claim-abort", methods=["POST"])
def disso_test_abort():
    try:
        gate = _require_auth()
        if gate:
            return gate
        gate = _require_any_session_internal(
            ["quick-test", "recipe-test"],
            "Forbidden. You do not have permission to abort tests.",
        )
        if gate:
            return gate
        user = data_service.get_current_user() or {}
        result = disso_test_service.claim_abort(user)
        return jsonify(result), 200 if result.get("ok") else 400
    except Exception as e:
        app.logger.exception("disso test abort")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/hardware/disso/temperature/live", methods=["GET"])
def disso_temp_live():
    try:
        # poll=1 triggers a one-shot UART #TEMP* (no background polling)
        do_poll = str(request.args.get("poll") or "").lower() in ("1", "true", "yes")
        if do_poll:
...
@app.route("/api/hardware/disso/status/live", methods=["GET"])
def disso_status_live():
    try:
        do_poll = str(request.args.get("poll") or "1").lower() in ("1", "true", "yes")
        if do_poll:
...
@app.route("/api/hardware/disso/stream", methods=["GET"])
def disso_temp_stream():
    try:
        return disso_temp_hardware.start_sse_stream()
    except Exception as e:
        app.logger.exception("disso stream")
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/hardware/disso/events", methods=["GET"])
def disso_cmd_events():
    """Poll async command-ESP events (ENT-TSML-VL, END-TEST, etc.)."""
    gate = _require_auth()
    if gate:
        return gate
    clear = str(request.args.get("clear") or "1").strip().lower() not in ("0", "false", "no")
    try:
        events = disso_cmd_hardware.pop_events() if clear else disso_cmd_hardware.peek_events()
...
def disso_recipe_upload():
    try:
        ...
        result = disso_cmd_hardware.upload_recipe(
            recipe,
            from_step_index=int(body.get("fromStepIndex") or 0),
            remaining_sec_in_step=body.get("remainingSecInStep"),
        )
        return jsonify(result), 200 if result.get("ok") else 400
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def disso_rpm_start():
    ...
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    return jsonify(disso_cmd_hardware.start_rpm(int(body.get("rpm") or 0))), 200
...
def disso_rpm_stop():
    ...
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    return jsonify(disso_cmd_hardware.stop_rpm(int(body.get("rpm") or 0))), 200
...
def disso_lift(action):
    ...
    if gate:
        return gate
    try:
        return jsonify(disso_cmd_hardware.lift(action)), 200
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
def disso_clean():
    ...
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    return jsonify(disso_cmd_hardware.clean(body.get("channel") or "A", body.get("volumeMl") or body.get("volume") or 5)), 200


@app.route("/api/hardware/disso/init", methods=["POST"])
@app.route("/api/hardware/disso/initialize", methods=["POST"])
def disso_hardware_init():
    gate = _require_session_internal(
        "settings",
        "Forbidden. You do not have permission to initialise hardware.",
    )
    if gate:
        return gate
    try:
        ...
        if (st or {}).get("active") and status in ("RUNNING", "PAUSED"):
            return jsonify({
                "ok": False,
                "error": "Cannot initialise hardware while a test is running or paused. Abort or complete the test first.",
            }), 409
        result = disso_cmd_hardware.initialise()
        ok = bool(result.get("ok"))
        _audit(
            None,
            None,
            "Hardware initialise",
            "ok" if ok else (result.get("error") or "failed"),
        )
        return jsonify(result), 200 if ok else 503
    except Exception as e:
        app.logger.exception("Hardware initialise failed")
        _audit(None, None, "Hardware initialise", str(e))
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/hardware/disso/beep", methods=["POST"])
def disso_hardware_beep():
    gate = _require_auth()
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    try:
        ...
        count = 1
    result = disso_cmd_hardware.beep(count)
    return jsonify(result), 200 if result.get("ok") else 503


@app.route("/api/hardware/disso/cal/temp", methods=["POST"])
def disso_cal_temp():
    gate = _require_auth()
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    try:
        target = str(body.get("target") or "BT").strip().upper()
        if target in ("V1", "V2", "V3", "V4", "V5", "V6", "VSL1", "VSL2", "VSL3", "VSL4", "VSL5", "VSL6"):
            return jsonify({"ok": False, "error": "Vessel calibration is not supported; use BT or EXT only"}), 400
        return jsonify(disso_cmd_hardware.cal_temp(target, float(body.get("value")))), 200
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400


@app.route("/api/hardware/disso/cal/sample-volume", methods=["POST"])
def disso_cal_sample():
    gate = _require_auth()
    if gate:
        return gate
    body = request.get_json(force=True, silent=True) or {}
    if body.get("phase") == "start" or body.get("value") is None:
        return jsonify(disso_cmd_hardware.cal_sample_start()), 200
    return jsonify(disso_cmd_hardware.cal_sample_value(float(body.get("value")))), 200
