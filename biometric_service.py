#!/usr/bin/env python3
"""
biometric_service.py - R307 fingerprint sensor service for Dissolution Tester.
Implements enrollment, identification, and template management over UART.
DT-parity: cancel-in-flight + Aura LED idle after capture/timeout/cancel.
"""
...
def capture_enroll_finger(buffer_id, timeout_sec=10.0):
    ...
    if not verify.get("ok"):
        return verify
    cap = _capture_to_buffer(buffer_id, timeout_sec=timeout_sec)
    if _cancel_requested.is_set():
        return {"ok": False, "error": "cancelled", "cancelled": True, "code": None}
    return cap
