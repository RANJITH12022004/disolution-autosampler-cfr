...
def _validate_dissolution_recipe(recipe_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    media = str(recipe_data.get("media") or "").strip()
    if not media:
    ...
    try:
        media_ph = float(recipe_data.get("mediaPh"))
        if media_ph < 0 or media_ph > 14:
            errors.append("Media pH must be between 0 and 14")
    except (TypeError, ValueError):
        errors.append("Media pH is required")
...
def process_recipe_form_data(form_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _is_dissolution_recipe(recipe):
        ...
        try:
            recipe["mediaPh"] = float(recipe.get("mediaPh"))
        except (TypeError, ValueError):
            recipe.pop("mediaPh", None)
