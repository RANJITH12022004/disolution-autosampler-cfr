...
def _validation_run_detail_pairs(run: Dict[str, Any]) -> list:
    ...
    if sub == "temperature":
        pairs.extend([
            ("Type", "Temperature"),
            ("Bath (°C)", _cell_str(run.get("bath"))),
            ("External (°C)", _cell_str(run.get("external"))),
            ("Measured (°C)", _cell_str(run.get("measured"))),
            ("Tolerance (± °C)", _cell_str(run.get("tolerance"))),
        ])
        ...
        pairs.extend([
            ...
            ("Live RPM", _cell_str(run.get("currentRpm") or run.get("tachometerRpm"))),
        ...
        pairs.extend([
            ...
            ("Acknowledged", "Yes" if run.get("acknowledged") else "No"),
        ])
    else:
        pairs.extend([
...
def _fmt_report_time_only(ts: Any) -> str:
    """HH:MM:SS from ISO-ish timestamp or already-formatted time."""
    if ts is None or ts == "":
        return "--"
    s = str(ts).strip()
    if re.match(r"^\d{1,2}:\d{2}(:\d{2})?$", s):
        parts = s.split(":")
        if len(parts) == 2:
            return "{:02d}:{:02d}:00".format(int(parts[0]), int(parts[1]))
        return "{:02d}:{:02d}:{:02d}".format(int(parts[0]), int(parts[1]), int(parts[2]))
    date_s, time_s = _split_ts_date_and_time(s)
    return time_s if time_s and time_s != "--" else "--"


def _append_step_details_two_col(lines: list, steps: list, width: int, thermal: bool) -> None:
    """Two-column Step / RPM / Duration layout."""
    if not isinstance(steps, list) or not steps:
    ...
    for i, st in enumerate(steps):
        if not isinstance(st, dict):
            continue
        rpm = st.get("rpm")
        dur = st.get("durationHms") or st.get("setTime") or st.get("duration") or st.get("time")
        if dur is None and st.get("durationSeconds") is not None:
            try:
                from report_service import format_duration_hhmmss
                dur = format_duration_hhmmss(int(st.get("durationSeconds")))
            except Exception:
                dur = str(st.get("durationSeconds"))
        normalized.append((i + 1, rpm, dur or "--"))
    if not normalized:
        lines.append("  (no steps)")
        return
    mid = (len(normalized) + 1) // 2
    left = normalized[:mid]
    right = normalized[mid:]
    if thermal:
        lines.append(f"{'St':>2} {'RPM':>4} {'Dur':>8}")
        for step, rpm, dur in normalized:
            lines.append(f"{step:02d} {_cell_str(rpm):>4} {_cell_str(dur):>8}")
        return
    header = f"{'Step':>4}  {'RPM':>5}  {'Duration':>8}   |   {'Step':>4}  {'RPM':>5}  {'Duration':>8}"
    lines.append(header)
    lines.append("-" * min(width, len(header) + 4))
    for i in range(mid):
        l = left[i] if i < len(left) else None
        r = right[i] if i < len(right) else None
        left_s = (
            f"{l[0]:02d}    {_cell_str(l[1]):>5}  {_cell_str(l[2]):>8}"
            if l
            else " " * 22
        )
        right_s = (
            f"{r[0]:02d}    {_cell_str(r[1]):>5}  {_cell_str(r[2]):>8}"
            if r
            else ""
        )
        if right_s:
            lines.append(f" {left_s}   |   {right_s}")
        else:
            lines.append(f" {left_s}")


def _append_test_report_details(lines: list, td: Dict[str, Any], report_data: Dict[str, Any], width: int, thermal: bool) -> None:
    """Append comments, step details, continuous TEST RESULTS, and completion line."""
    dash = "" if thermal else ("-" * width)
    recipe = report_data.get("recipe") if isinstance(report_data.get("recipe"), dict) else {}
    if not recipe and isinstance(td, dict):
        recipe = td.get("recipe") if isinstance(td.get("recipe"), dict) else {}
    ...
    if not steps and isinstance(recipe, dict):
    ...
    if thermal:
        ...
        lines.extend(["", "STEP DETAILS", dash if dash else ""])
    _append_step_details_two_col(lines, steps, width, thermal)

    if thermal:
        lines.extend(["", "Comments: {}".format(remarks if remarks not in (None, "") else "____________________")])
    else:
        lines.extend(["", "Comments: {}".format(remarks if remarks not in (None, "") else "____________________"), ""])
    ...
    if thermal:
        lines.extend(["", "TEST RESULTS", f"{'#':>3} {'Time':>8} {'RPM':>4} {'Set':>5} {'Bath':>5} {'Dev':>5}"])
    else:
        eq = _section_sep("=", width, False)
        lines.extend(["", eq, "TEST RESULTS", dash if dash else ""])
        lines.append(f"{'Sl.No.':>6}  {'Time':>8}  {'RPM':>5}  {'Set Temp.':>9}  {'Bath Temp.':>10}  {'Deviation':>9}")
        if dash:
            lines.append(dash)
    ...
    if temp_log:
        for i, row in enumerate(temp_log):
            ...
            if bath is not None:
                try:
                    bath_s = "{:.1f}".format(float(bath))
                except (TypeError, ValueError):
                    bath_s = _cell_str(bath)
            else:
                bath_s = "--"
            if dev is not None:
                try:
            ...
            if thermal:
                lines.append(
                    f"{i + 1:03d} {t:>8} {_cell_str(rpm):>4} {set_s:>5} {bath_s:>5} {dev_s:>5}"
                )
            else:
                lines.append(
...
def _format_report_text(report_data: Dict[str, Any], width: int = A4_TEXT_WIDTH) -> str:
    ...
    title = (
        "DISSOLUTION VALIDATION REPORT"
        if rtype == "validation"
        else "DISSOLUTION CALIBRATION REPORT"
        if rtype == "calibration"
        else "DISSOLUTION TEST REPORT"
    )
    lines: list = []
    if thermal:
    ...
    if rtype == "validation":
        ...
        td_cal = td if isinstance(td, dict) else {}
        subtype = str(
            td_cal.get("calibrationSubtype")
            or report_data.get("calibrationSubtype")
            or "temperature"
        ).strip().lower()
        start_ts = (
            td_cal.get("calibrationStartTime")
            or td_cal.get("testStartTime")
            or report_data.get("calibrationStartTime")
            or report_data.get("createdAt")
        )
        end_ts = (
            td_cal.get("calibrationEndTime")
            or td_cal.get("testEndTime")
            or report_data.get("completedAt")
            or report_data.get("createdAt")
        )
        status_label = str(td_cal.get("status") or report_data.get("status") or "Completed")
        if thermal:
            lines.extend(["", "CALIBRATION RESULTS", f"Type: {subtype}", f"Status: {status_label}"])
            lines.append(f"Start: {_format_ts_readable(start_ts)}")
            lines.append(f"End: {_format_ts_readable(end_ts)}")
            channels = td_cal.get("temperatureChannels") or report_data.get("temperatureChannels") or []
            if isinstance(channels, list):
                for ch in channels:
                    if not isinstance(ch, dict):
                        continue
                    ...
                    live = ch.get("live")
                    actual = ch.get("actual") if ch.get("actual") is not None else ch.get("reference")
                    lines.append(f"{label}: live={live} actual={actual}")
            lines.append("")
        else:
            ...
            _append_two_column_pairs(
                ...
                [
                    ("Type", subtype),
                    ("Status", status_label),
                    ("Start Time", _format_ts_readable(start_ts)),
                    ("End Time", _format_ts_readable(end_ts)),
                ],
                width,
            )
            channels = td_cal.get("temperatureChannels") or report_data.get("temperatureChannels") or []
        ...
        sample_drop = td.get("mode") or recipe.get("mode") or "--"
        set_temp = td.get("temperature") if td.get("temperature") is not None else recipe.get("temperature")
        try:
            set_temp_s = "{:.1f} C".format(float(set_temp)) if set_temp is not None else "--"
        except (TypeError, ValueError):
            set_temp_s = "{} C".format(set_temp) if set_temp is not None else "--"
        step_count = td.get("stepCount")
        if step_count is None:
            steps_list = td.get("steps") or recipe.get("steps") or []
            step_count = len(steps_list) if isinstance(steps_list, list) else "--"
        media_vol = td.get("mediaVolume") or recipe.get("mediaVolume") or "--"
        media_ph = td.get("mediaPh") if td.get("mediaPh") is not None else recipe.get("mediaPh")
        if thermal:
            lines.extend(
                [
                    "TEST DETAILS",
                    f"Test Date: {start_date}  Time: {start_time}",
                    f"Recipe: {recipe.get('productName') or td.get('productName') or '--'}",
                    f"Test Method: {usp}",
                    f"Set Temp: {set_temp_s}",
                    f"Sample Drop: {sample_drop}",
                    "",
                    "PRODUCT DETAILS",
                    f"Product: {recipe.get('productName', td.get('productName', 'N/A'))}",
                    f"Batch: {recipe.get('batchNumber', td.get('batchNumber', 'N/A'))}",
                    f"Steps: {step_count}",
                    f"Media Vol: {media_vol}",
                    f"Media PH: {media_ph if media_ph is not None else '--'}",
                ]
            )
        else:
            lines.extend(["", "TEST DETAILS", sep_dash])
            _append_two_column_pairs(
                lines,
                [
                    ("Test Date", start_date),
                    ("Time", start_time),
                    ("Recipe No.", recipe.get("productName") or recipe.get("name") or td.get("productName") or "--"),
                    ("Test Method", usp),
                    ("Set Temp", set_temp_s),
            ...
            lines.extend(["", "PRODUCT DETAILS", sep_dash])
            _append_two_column_pairs(
                lines,
                [
                    ("Product Name", recipe.get("productName", td.get("productName", "N/A"))),
                    ("Batch Number", recipe.get("batchNumber", td.get("batchNumber", "N/A"))),
                    ("Total Steps No", step_count),
                    ("Media Volume", media_vol),
                    ("Media PH", media_ph if media_ph is not None else "--"),
                ],
                width,
            )
