...
var PROCEDURE_VALIDATION_THERMOMETER = [
    'Please Measure The Bath And Vessel Temperature With A Certified Thermometer.'
];
var PROCEDURE_CALIBRATION_TEMPERATURE = [
    '1. Allow the bath and external sensors to stabilize near the operating temperature.',
    '2. Measure Bath and External temperature with a certified reference thermometer.',
    '3. Enter the Actual (reference) values for Bath and External (valid range 15–55 °C).',
    '4. Press Calibrate to apply the correction to the instrument.',
    '5. Confirm that live readings match the reference after calibration completes.'
];
...
function goToPage(pageName) {
    ...
    if (pageName === 'validate-type-select' || pageName === 'calibration' ||
        ...
        pageName === 'cleaning-cycle' || pageName === 'ip-config' || pageName === 'ip-config-result') {
...
function goBack() {
    ...
    if (pageId === 'page-create-recipe-step1') {
        ...
        pageId === 'page-temperature-validation' || pageId === 'page-rpm-validation' ||
...
function startQuickTest() {
    var ids = [
        ...
        'quick-power-failure', 'quick-media-volume', 'quick-media-ph'
    ];
    ...
    var autoDispEl = document.getElementById('quick-auto-dispense');
    if (modeEl) modeEl.value = 'Auto';
    if (uspEl) uspEl.value = 'USP 1';
    if (repEl) repEl.value = 'Yes';
    if (autoDispEl) autoDispEl.value = 'No';
    if (typeof syncAutoDispenseFieldVisibility === 'function') syncAutoDispenseFieldVisibility('quick');
    var list = document.getElementById('quick-create-recipe-steps-list');
    if (list) list.innerHTML = '';
...
function startQuickTestRunFromParams() {
    ...
    var autoDispense = (usp === 'USP 2' && autoDispEl && String(autoDispEl.value) === 'Yes');
    ...
    if (mode !== 'Auto' && mode !== 'Manual') {
        showAppModal('Please select mode (Auto or Manual).', 'Quick Test');
        return;
    }
...
function startRecipeCreation() {
    ...
    var autoDispEl = document.getElementById('recipe-auto-dispense');
    if (modeEl) modeEl.value = 'Auto';
    if (uspEl) uspEl.value = 'USP 1';
    if (repEl) repEl.value = 'Yes';
    if (autoDispEl) autoDispEl.value = 'No';
    if (typeof syncAutoDispenseFieldVisibility === 'function') syncAutoDispenseFieldVisibility('recipe');
...
function syncAutoDispenseFieldVisibility(which) {
    var prefix = which === 'quick' ? 'quick' : 'recipe';
    var uspEl = document.getElementById(prefix + '-usp');
    var wrap = document.getElementById(prefix + '-auto-dispense-wrap');
    if (!wrap) return;
    var usp = uspEl ? String(uspEl.value || '') : '';
    wrap.style.display = (usp === 'USP 2') ? '' : 'none';
}
...
function validateCreateRecipeStep1Fields() {
    ...
    if (mode !== 'Auto' && mode !== 'Manual') {
        showAppModal('Please select mode (Auto or Manual).', 'Create Recipe');
        return null;
    }
    if (usp !== 'USP 1' && usp !== 'USP 2') {
...
function initRpmValidationPage() {
    ...
    if (tachEl && !tachEl.value) tachEl.value = '';
    _rpmValLive = null;
    stopRpmValidationLive();
}
...
function loadReports(filterType) {
    ...
    if (filterType === 'test' || filterType === 'validation') {
        ...
        if (!list.length) {
            ...
            list.forEach(function (r, i) {
                ...
                if (!name && r.type === 'validation') {
                    var vst = r.validationSubtype;
                    var vlabel = 'USP';
                    name = 'Validation - ' + vlabel;
                }
...
function populateReportPreview(preview) {
    ...
    if (reportType === 'validation') {
        ...
        if (bodyEl) {
            var subtype = String(td.validationSubtype || preview.validationSubtype || '').toLowerCase();
            var startDateStr = formatReportDate(td.validationStartTime || td.testStartTime || preview.validationStartTime || preview.createdAt);
            var dateStr = formatReportDate(td.validationEndTime || td.testEndTime || td.completedAt || preview.completedAt || preview.createdAt);
            ...
            if (subtype === 'temperature') {
                ...
                rows.push('<tr><th>Type</th><td>' + (td.usp || subtype || 'Validation') + '</td><th>Status</th><td>' + status + '</td></tr>');
            }
    ...
    var startStr = formatReportDate(td.testStartTime || td.validationStartTime || preview.validationStartTime || preview.createdAt);
    var endStr = formatReportDate(td.testEndTime || td.validationEndTime || preview.completedAt || preview.createdAt);
    var generatedStr = formatReportDate(preview.createdAt || td.createdAt || td.testEndTime || preview.completedAt);
    setReportEl('report-test-start', startStr);
    setReportEl('report-generated', generatedStr);
    ...
    if (tbody) {
        ...
        var stepsMeta = td.steps || [];
        var rows = [];
        var runDetails = td.runDetails || td.runSummary || '';
        if (!runDetails) {
            var detailParts = [];
            if (td.mode) detailParts.push('Mode: ' + td.mode);
            if (td.usp || td.uspMode) detailParts.push('USP: ' + (td.usp || td.uspMode));
            if (td.temperature != null) detailParts.push('Temp: ' + td.temperature + ' °C');
            if (td.sampleVolume) detailParts.push('Sample: ' + td.sampleVolume);
            if (td.rinseVolume) detailParts.push('Flush: ' + td.rinseVolume);
            if (td.batchSize) detailParts.push('Batch size: ' + td.batchSize);
            if (td.arNumber) detailParts.push('AR: ' + td.arNumber);
            if (td.replenishment != null) detailParts.push('Replenishment: ' + td.replenishment);
            if (td.powerFailure != null) detailParts.push('Power Failure: ' + formatPowerFailureDisplay(td.powerFailure));
            runDetails = detailParts.join(', ');
        }
        if (runDetails) {
            rows.push('<tr><th colspan="1">RUN DETAILS</th><td colspan="3">' + escapeReportHtml(runDetails) + '</td></tr>');
        }

        if (stepCount > 0) {
            for (var i = 0; i < stepCount; i++) {
                var r = results[i] || {};
                var meta = stepsMeta[i] || {};
                var rpm = (r.rpm != null && r.rpm !== '') ? r.rpm : meta.rpm;
                var setTime = (r.setTime != null && r.setTime !== '') ? r.setTime
                    : (r.durationHms || meta.durationHms || meta.time || '--');
                if (setTime === '--' && (r.durationSeconds != null || meta.durationSeconds != null)) {
                    var sec = parseInt(r.durationSeconds != null ? r.durationSeconds : meta.durationSeconds, 10) || 0;
                    var hh = Math.floor(sec / 3600);
                    var mm = Math.floor((sec % 3600) / 60);
                    var ss = sec % 60;
                    setTime = (hh < 10 ? '0' : '') + hh + ':' + (mm < 10 ? '0' : '') + mm + ':' + (ss < 10 ? '0' : '') + ss;
                }
                var status = (r.status != null && r.status !== '') ? r.status
                    : ((r.resultText != null && r.resultText !== '') ? r.resultText : '__');
                rows.push(
                    '<tr>' +
                        '<td>' + (i + 1) + '</td>' +
                        '<td>' + (rpm != null && rpm !== '' ? rpm : '__') + '</td>' +
                        '<td>' + setTime + '</td>' +
                        '<td>' + status + '</td>' +
                    '</tr>'
                );
            ...
            if (!rows.length) rows.push('<tr><td colspan="4">No test data</td></tr>');
            tbody.innerHTML = rows.join('');
        }
    }
...
function loadRecipeForEdit() {
    ...
    apiRequest(API_BASE + '/api/data/recipes/' + id).then(function (data) {
        ...
        if (typeof isDissolutionRecipe === 'function' && isDissolutionRecipe(r)) {
            ...
            if (modeEl) modeEl.value = (r.mode === 'Manual') ? 'Manual' : 'Auto';
            ...
            if (autoDispEl) autoDispEl.value = r.autoDispense ? 'Yes' : 'No';
            if (typeof syncAutoDispenseFieldVisibility === 'function') syncAutoDispenseFieldVisibility('recipe');
...
function promptAutoDispenseSelection(recipe) {
    // Dissolution: keep recipe Auto Dispense from USP 2 form; no Friability prompt.
...
var SYSTEM_SETTINGS_DEFAULTS = {
    ...
    printIntervalMinutes: null
};

function _sysSetSelectValue(id, value) {
    var el = document.getElementById(id);
    if (!el) return;
...
function setSystemSettingsForm(settings) {
    ...
    _sysSetSelectValue('sys-beep', s.beep);
    _sysSetSelectValue('sys-temp-tolerance', s.tempTolerance);
    var valueEl = document.getElementById('sys-temp-tolerance-value');
    if (valueEl) {
    ...
    _sysSetSelectValue('sys-print-interval', s.printInterval);
    var printValueEl = document.getElementById('sys-print-interval-value');
    if (printValueEl) {
        printValueEl.value = (s.printIntervalMinutes != null && !isNaN(Number(s.printIntervalMinutes)))
            ? String(s.printIntervalMinutes)
            : '';
    }
...
function collectSystemSettingsForm() {
    ...
    var printSelect = (document.getElementById('sys-print-interval') || {}).value || 'Disable';
    var printValueEl = document.getElementById('sys-print-interval-value');
    var rawPrintValue = printValueEl ? String(printValueEl.value || '').trim() : '';
    var parsedPrintValue = rawPrintValue === '' ? null : parseInt(rawPrintValue, 10);
    return {
        ...
        printInterval: printSelect,
        printIntervalMinutes: printSelect === 'Enable' ? parsedPrintValue : null
    };
}

function initSystemSettingsPage() {
    var toleranceSelect = document.getElementById('sys-temp-tolerance');
    if (toleranceSelect && !toleranceSelect.dataset.bound) {
        toleranceSelect.dataset.bound = '1';
        toleranceSelect.addEventListener('change', syncTempToleranceFieldVisibility);
    }
    var printIntervalSelect = document.getElementById('sys-print-interval');
    if (printIntervalSelect && !printIntervalSelect.dataset.bound) {
        printIntervalSelect.dataset.bound = '1';
        printIntervalSelect.addEventListener('change', syncPrintIntervalFieldVisibility);
    }
...
function saveSystemSettings() {
    ...
    if (payload.printInterval === 'Enable') {
        var mins = payload.printIntervalMinutes;
        if (mins == null || isNaN(mins) || mins < 2 || mins > 60) {
            showAppModal('Please enter a print interval between 2 and 60 minutes.', 'Test Settings');
            return;
        }
    }
    ...
    apiRequest(API_BASE + '/api/data/system-settings', { method: 'POST', body: payload })
        ...
        .catch(function (err) {
            console.error('Failed to save test settings', err);
            showAppModal('Failed to save test settings.', 'Test Settings');
        })
        .finally(function () {
...
function setSysinfoStatus(id, tileId, val) {
    var el = document.getElementById(id);
    var tile = document.getElementById(tileId);
    var text = (val === null || val === undefined || val === '') ? '—' : String(val);
    if (el) el.textContent = text;
    if (!tile) return;
    tile.classList.remove('is-ok', 'is-error');
    var low = text.toLowerCase();
    if (low === 'ok') tile.classList.add('is-ok');
    else if (low === 'error' || low === 'fail' || low === 'fault') tile.classList.add('is-error');
}

function setSystemInfoLoading() {
    setSysinfoBath('…');
    setSysinfoStatus('sysinfo-float-switch', 'sysinfo-tile-float-switch', '…');
    setSysinfoStatus('sysinfo-heater-sensor', 'sysinfo-tile-heater-sensor', '…');
    setSysinfoStatus('sysinfo-pump', 'sysinfo-tile-pump', '…');
    setSysinfoStatus('sysinfo-sample-collector', 'sysinfo-tile-sample-collector', '…');
}

function applySystemInfoData(data) {
    setSysinfoBath(data && (data.bath || data.bath_c));
    setSysinfoStatus('sysinfo-float-switch', 'sysinfo-tile-float-switch', data && data.float_switch);
    setSysinfoStatus('sysinfo-heater-sensor', 'sysinfo-tile-heater-sensor', data && data.heater_sensor);
    setSysinfoStatus('sysinfo-pump', 'sysinfo-tile-pump', data && data.pump);
    setSysinfoStatus(
        'sysinfo-sample-collector',
        'sysinfo-tile-sample-collector',
        data && (data.sample_collector || data.collector)
    );
}

function setSystemInfoFallback() {
    applySystemInfoData({
        bath: '37.1 °C',
        float_switch: 'Ok',
        heater_sensor: 'Ok',
...
function loadSystemInfo() {
    ...
    apiRequest(API_BASE + '/api/system/info', { method: 'GET' })
        ...
        .catch(function () { setSystemInfoFallback(); });
}
...
function updateCleaningUI() {
    ...
    var stopBtn = document.getElementById('cleaning-stop-btn');
    ...
    if (startBtn) startBtn.disabled = _cleaningRunning;
    if (stopBtn) stopBtn.disabled = !_cleaningRunning;
...
/* ── IP Config (Settings / factory support) ──────────────────── */

function initIpConfigPage() {
    var loadingEl = document.getElementById('ip-config-loading-msg');
    var userEl = document.getElementById('ip-config-user');
    var passEl = document.getElementById('ip-config-password');
    var btnBack = document.getElementById('ip-config-btn-back');
    var btnSubmit = document.getElementById('ip-config-btn-submit');
...
function initIpConfigResultPage() {
    var el = document.getElementById('ip-config-result-ip');
    if (!el) return;
    var raw = '';
    try {
        raw = sessionStorage.getItem('ipConfigResultIp') || '';
    } catch (e) {
        raw = '';
    }
...
function ipConfigVerifyFetch(username, password) {
    return fetch((typeof API_BASE === 'string' ? API_BASE : '') + '/api/support/factory/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: username, password: password })
...
function submitIpConfigLogin() {
    ...
    if (!username || !password) {
        if (typeof showAppModal === 'function') {
            showAppModal('Please enter factory support ID and password.', 'IP Config');
        } else {
    ...
    ipConfigVerifyFetch(username, password)
        .then(function (res) {
            var minMs = 1200;
        ...
        .then(function (res) {
            if (res && res.ok) {
                ...
                try {
                    sessionStorage.setItem('ipConfigResultIp', text);
                } catch (e) {}
                if (typeof logAuditEvent === 'function') {
                    logAuditEvent('IP Config viewed', text, {
                        eventType: 'lifecycle',
                        entityType: 'settings'
                    });
                }
                goToPage('ip-config-result');
            } else {
        ...
        .catch(function (e) {
            restoreForm();
            if (typeof showAppModal === 'function') {
                showAppModal(e.message || 'Sign-in failed', 'IP Config');
            } else {
...
function runTemperatureCalibration() {
    ...
    var vesselActuals = [];
    var v;
    for (v = 1; v <= TEMP_CAL_VESSEL_COUNT; v++) {
        vesselActuals.push(getTempCalActualValue('temp-cal-v' + v + '-actual'));
    }
    function isValidTemp(val) {
        return !isNaN(val) && val >= TEMP_CAL_MIN && val <= TEMP_CAL_MAX;
    }
    var allValid = isValidTemp(bathActual) && isValidTemp(externalActual);
    for (v = 0; v < vesselActuals.length; v++) {
        if (!isValidTemp(vesselActuals[v])) allValid = false;
    }
    if (!allValid) {
        if (typeof showAppModal === 'function') {
            showAppModal(
                'Please enter Actual temperatures between ' + TEMP_CAL_MIN + ' and ' + TEMP_CAL_MAX +
                ' °C for bath, external, and all vessels.',
                'Calibration'
            );
        }
        return;
    ...
    var vesselTxts = vesselActuals.map(function (val) { return val.toFixed(1); });
    var cmd = 'temp,calibrate,' + bathTxt + ',' + externalTxt + ',' + vesselTxts.join(',');

    if (typeof logAuditEvent === 'function') {
        logAuditEvent(
            'Temperature calibration started',
            'Bath ' + bathTxt + ' °C, External ' + externalTxt + ' °C, Vessels ' + vesselTxts.join('/') + ' °C',
            {
                eventType: 'lifecycle',
    ...
    apiRequest(API_BASE + '/api/hardware/command', {
        ...
        if (result && result.ok !== false && !result.error) {
            ...
            var err = (result && result.error) ? result.error : 'Calibration failed.';
            setTempCalStatus(err, 'is-error');
        }
    }).catch(function () {
        _tempCalRunning = false;
        updateTemperatureCalibrationUI();
        setTempCalStatus('Calibration request failed. Check hardware connection.', 'is-error');
    });
}
...
function _dtSyncEquipVisuals() {
    ...
    var heaterOn = !!(dt && (dt.running || dt.paused || dt.preheating || dt.heaterForcedOn || (dt.preheatDone && dt.needsPreheat)));
    var pumpOn = !!(dt && (dt.running || dt.paused || dt.preheating));
...
function _dtResetPreheatUi() {
    ...
    if (!dt) { if (preheatBtn) preheatBtn.style.display = 'none'; return; }
    if (preheatBtn) {
        if (dt.needsPreheat) {
            preheatBtn.style.display = '';
            preheatBtn.disabled = !!dt.preheatDone;
            preheatBtn.textContent = dt.preheatDone ? 'Preheat Done' : (dt.preheating ? 'Preheating…' : 'Preheat');
            preheatBtn.classList.toggle('dt-preheat-done', !!dt.preheatDone);
        } else {
            preheatBtn.style.display = 'none';
            preheatBtn.disabled = false;
            preheatBtn.classList.remove('dt-preheat-done');
            preheatBtn.textContent = 'Preheat';
        }
    }
    if (startBtn && !dt.running && !dt.paused) {
        var block = !!(dt.needsPreheat && !dt.preheatDone);
        startBtn.disabled = block || dt.preheating;
        if (block || dt.preheating) startBtn.classList.add('dt-start-grey');
    }
    _dtSyncEquipVisuals();
}
...
function dissolutionPreheatStart() {
    var dt = _dissolutionTest;
    if (!dt || !dt.needsPreheat || dt.preheatDone || dt.preheating || dt.running) return;
    dt.preheating = true;
    dt.heaterForcedOn = true;
    _dtResetPreheatUi();
    _dtSetStatus('Preheating… please wait', 'ready');
    _dtStopPreheatTimer();
    dt.preheatTimerId = setTimeout(function () {
...
function initDissolutionTestRun(recipe) {
    ...
    _dtStopTimer();
    var usp = recipe.usp || recipe.uspMode || '';
    var needsPreheat = String(usp) === 'USP 2';
    _dissolutionTest = {
        recipe: recipe,
        ...
        testStartTime: null,
        reportSaved: false,
        needsPreheat: needsPreheat,
        preheating: false,
        preheatDone: !needsPreheat,
        preheatTimerId: null,
        heaterForcedOn: false
    };
    window._dissolutionTest = _dissolutionTest;

    _dtSetText('dt-recipe-name', recipe.productName || recipe.name || '--');
    ...
    _dtSetText('dt-media', recipe.mediaVolume || recipe.media || '--');
    _dtSetText('dt-media-ph', recipe.mediaPh != null ? recipe.mediaPh : '--');
    var adChip = _dtEl('dt-auto-dispense-chip');
    var adVal = _dtEl('dt-auto-dispense');
    if (needsPreheat) {
        if (adChip) adChip.style.display = '';
        if (adVal) adVal.textContent = recipe.autoDispense ? 'Yes' : 'No';
    } else if (adChip) {
        adChip.style.display = 'none';
    }
...
function _dtSetControlsIdle() {
    ...
    if (startBtn) { startBtn.style.display = ''; startBtn.textContent = 'Start'; startBtn.disabled = false; startBtn.classList.add('dt-start-grey'); }
...
function _dtSetControlsRunning() {
    ...
    if (preheatBtn) { preheatBtn.disabled = true; preheatBtn.classList.add('dt-preheat-done'); }
    _dtSyncStirrerLock();
}
...
function _dtSetControlsPaused() {
    ...
    var pauseBtn = _dtEl('dt-pause-btn');
    var abortBtn = _dtEl('dt-abort-btn');
    if (startBtn) { startBtn.style.display = ''; startBtn.textContent = 'Resume'; startBtn.disabled = false; startBtn.classList.remove('dt-start-grey'); }
    if (pauseBtn) pauseBtn.style.display = 'none';
    if (abortBtn) abortBtn.style.display = '';
    _dtSyncStirrerLock();
}
...
function _dtSyncStirrerLock() {
    var dt = _dissolutionTest;
    var locked = !!(dt && dt.running && !dt.paused);
...
function stirrerUnitCommand(cmd) {
    ...
    var dt = _dissolutionTest;
    if (dt && dt.running && !dt.paused) {
        showAppModal('Pause the test before moving the stirrer unit.', 'Stirrer Unit');
        return;
    }
    ...
    if (typeof logAuditEvent === 'function') {
        logAuditEvent('Stirrer unit ' + cmd, 'Stirrer command: ' + (labels[cmd] || cmd), { eventType: 'lifecycle' });
    }
}
...
function _dtOnStepComplete() {
    ...
    _dtSetControlsIdle();
    var startBtn = _dtEl('dt-start-btn');
    if (startBtn) { startBtn.textContent = 'Start'; startBtn.disabled = true; }
    var pctEl = _dtEl('dt-progress-pct');
    var fillEl = _dtEl('dt-progress-fill');
...
function _dtBuildCompletionReportPayload(opts) {
    ...
    var detailParts = [];
    if (recipe.mode) detailParts.push('Mode: ' + recipe.mode);
    ...
    if (recipe.mediaVolume || recipe.media) detailParts.push('Media: ' + (recipe.mediaVolume || recipe.media));
    if (recipe.mediaPh != null) detailParts.push('Media pH: ' + recipe.mediaPh);
    if (recipe.autoDispense) detailParts.push('Auto Dispense: Yes');
    ...
    return {
        ...
        testData: {
            ...
            media: recipe.mediaVolume || recipe.media,
            mediaPh: recipe.mediaPh,
            autoDispense: !!recipe.autoDispense,
...
function dissolutionTestBack() {
    if (_dissolutionTest && _dissolutionTest.running && !_dissolutionTest.paused) {
        showAppModal('Pause or abort the test before leaving.', 'Test');
        return;
    }
    _dtStopTimer();
    if (_dissolutionTest) { _dissolutionTest.running = false; _dissolutionTest.paused = false; }
    goToPage('manage-recipes');
}
...
function cleanupDissolutionTestOnLeave() {
    _dtStopPreheatTimer();
    _dtStopTimer();
    if (_dissolutionTest) { _dissolutionTest.running = false; _dissolutionTest.paused = false; _dissolutionTest.preheating = false; }
}
...
function renderVesselTemperaturePage() {
    ...
    var warnDelta = 0.5;
    var vessels = [
        { id: 1, current: setpoint, max: setpoint + 0.2, min: setpoint - 0.2 },
        { id: 2, current: setpoint + 0.1, max: setpoint + 0.3, min: setpoint - 0.1 },
        { id: 3, current: setpoint - 0.1, max: setpoint + 0.1, min: setpoint - 0.3 },
        { id: 4, current: setpoint + 0.2, max: setpoint + 0.4, min: setpoint - 0.1 },
        { id: 5, current: setpoint, max: setpoint + 0.2, min: setpoint - 0.2 },
        { id: 6, current: setpoint + 1.1, max: setpoint + 1.2, min: setpoint + 0.5 }
    ];
